...
class CartesianGraspApproach:
    ...
    def pose_callback(self, message):
        ...
        try:
            ...
            if not retreat_plan.joint_trajectory.points:
                raise RuntimeError(
                    "Cartesian retreat trajectory has no points"
                )
            ...
            retreat_success = self.arm_group.execute(
                retimed_retreat,
                wait=True,
            )
            ...
            rospy.loginfo(
                "Cartesian retreat completed successfully."
            )

            rospy.loginfo(
                "Grasp, close and retreat sequence completed."
            )

            rospy.signal_shutdown(
                "One grasp, close and retreat sequence executed."
            )
...
def main():
    ...
    rospy.init_node(
        "cartesian_grasp_close_retreat_node"
    )
